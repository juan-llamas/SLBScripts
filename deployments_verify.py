...
def main(argv):  
    ...
    argv = sys.argv[1:]
    tenant = argv[0]
    command_lin = "sudo /opt/OV/bin/ovc -status"
        ...
        for vm in current_deploys.json()["deployments"]:
            if re.match(r"^filesync-(.+)", vm['name']) or vm['name'] in deployments:
                ...
                if command_var.json()["isLinux"] is True:
                    print(f'{vm["name"]}')
                    command = os.system(f'gcloud compute ssh linuxadminuser@{command_var.json()["name"]} --command="{command_lin}" --project={tenant} --zone={command_var.json()["zone"]} --tunnel-through-iap --quiet')
                    border='-'
                    line = border * (80)
                    print(command)
                    print(line)
        ...
        for vm in current_deploys.json()["deployments"]:
            if re.match(r"^filesync-(.+)", vm['name']) or vm['name'] in deployments:
                ...
                if command_var.json()["isLinux"] is True:
                    ...
                    command = os.system(f'az vm run-command invoke -g {tenant} -n {command_var.json()["name"]} --command-id RunShellScript --scripts "{command_lin}"')
                    border='-'
                    line = border * (80)
                    print(command)
                    print(line)
